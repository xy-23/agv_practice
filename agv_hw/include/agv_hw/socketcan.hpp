#ifndef AGV_HW__SOCKETCAN_HPP
#define AGV_HW__SOCKETCAN_HPP
...
public:
  ...
  ~SocketCAN()
  {
    ...
    if (read_thread_.joinable()) read_thread_.join();
    close();
    RCLCPP_INFO(rclcpp::get_logger("SocketCAN"), "Destructed.");
  }
...
private:
  ...
  void open()
  {
    ...
    read_thread_ = std::thread([this]() {
      ...
      while (!quit_) {
        try {
          ...
          RCLCPP_WARN(rclcpp::get_logger("SocketCAN"), "Failed to read! %s", e.what());
    ...
    RCLCPP_INFO(rclcpp::get_logger("SocketCAN"), "Opened.");
  }
  ...
  void try_open()
  {
    try {
      ...
      RCLCPP_WARN(rclcpp::get_logger("SocketCAN"), "Failed to open! %s", e.what());
    }
  }
